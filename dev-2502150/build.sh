...
__main() {
    ...
    _dockerfile=$(
        cat <<"EOF"
FROM gcr.io/etcd-development/etcd:v3.5.18 AS etcd
...
RUN set -eux; \
    ...
    curl -Lo - "https://golang.google.cn/dl/go1.24.0.linux-amd64.tar.gz" | tar zxf - -C /usr/local/;
...
RUN set -eux; \
    ...
    git clone https://github.com/jimhester/per-directory-history.git ~/.oh-my-zsh/custom/plugins/per-directory-history; \
...
RUN set -eux; \
    ...
    apt-get update; apt-get install -y --no-install-recommends \
        ...
        cron  direnv shellcheck fzf zfsutils-linux \
...
ghcr.io/lwmacct/250209-cr-vscode:dev-2502150
